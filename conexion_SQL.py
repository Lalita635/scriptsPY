...
class Conectar():
    ...
    def CrearTabla(self):
        if self.conexion.is_connected():
            segundos = 5
            print('ENTRO!!!')
            nombre = input('Ingrese el nombre de la tabla: ')
            lista_columnas = []
            otra_columna = 1
            
            while otra_columna == 1:
                columna = input('Ingrese el nombre de la columna: ')
                tipo = input('Indique el tipo de valores de la columna: ')
                print('Indique la opción correcta: \n\tLa columna es: ')
                clave_primaria = int(input('Es clave Primaria? (1.Si o 2.No): '))
                nulo = int(input('Es un valor requerido, es decir que no puede quedar vacío? (1.Si o 2.No): '))
                incremento = int(input('Se genera automáticamente? (1.Si o 2.No): '))
                
                if clave_primaria == 1 and nulo == 1 and incremento == 1:
                    valor = columna + " " + tipo + " PRIMARY KEY " + "NOT NULL " + "AUTO_INCREMENT"
                elif clave_primaria == 1 and nulo == 1 and incremento == 2:
                    valor = columna + " " + tipo + " PRIMARY KEY " + "NOT NULL"
                elif clave_primaria == 1 and nulo == 2 and incremento == 1:
                    valor = columna + " " + tipo + " PRIMARY KEY " + "AUTO_INCREMENT"
                elif clave_primaria == 1 and nulo == 2 and incremento == 2:
                    valor = columna + " " + tipo + " PRIMARY KEY"
                elif clave_primaria == 2 and nulo == 1 and incremento == 1:
                    valor = columna + " " + tipo + " NOT NULL " + "AUTO_INCREMENT"
                elif clave_primaria == 2 and nulo == 2 and incremento == 1:
                    valor = columna + " " + tipo + " AUTO_INCREMENT"
                elif clave_primaria == 2 and nulo == 1 and incremento == 2:
                    valor = columna + " " + tipo + " NOT NULL"
                elif clave_primaria == 2 and nulo == 2 and incremento == 2:
                    valor = columna, tipo
                
                print('Quedo asi: ', valor)
                
                lista_columnas.append(valor)
                
                print('\tDesea agregar otra columna? (1.Si o 2.No)')
                otra_columna = int(input(': '))

            #CLAVES FOREANEAS
            lista_foraneas = []

            bandera = int(input('Desea establecer una clave foranea? (1.Si o 2.No): '))
            while bandera == 1:
                foranea = input('Ingrese el nombre de la clave foranea: ')
                referencia = input('Ingrese la referencia: ')
                clave_foranea =  "FOREIGN KEY (" + foranea + ") REFERENCES " + referencia
                lista_foraneas.append(clave_foranea)
                
                bandera = int(input('Desea establecer otra clave foranea? (1.Si o 2.No): '))
            
            if lista_foraneas:
                lista_columnas.extend(lista_foraneas)
            
            #convertimos a str
            columnas = ', '.join(map(str, lista_columnas))
            
            #SENTENCIA
            sentenciaSQL = "CREATE TABLE " + nombre + " (" + columnas + ");"
...
def menu():
    ...
    print("\t4. Agregar datos a una tabla")
    print("\t5. Buscar datos en la Base de Datos")
    print("\t6. Actualizar datos en una tabla")
    print("\t7. Borrar datos de una tabla")
    print("\t8. Contactar al desarrollador")
    print("\t9. Salir")
...
if __name__ == '__main__':
    ...
    while not bandera:
        ...
        if opcion == '1':
            ...
            conexion1.CrearTabla()
            time.sleep(segundos)
            
        elif opcion == '4':
            nombre_tabla = input('Inserte el nombre de la tabla: ')
